...
- **`skills/sell-slice/scripts/verify-frontend-statics.sh`**, a bundled deterministic static gate that `slice-verifier` runs as a **sub-check of its existing `design_system` key**. No new atomic check key, so verify-once (C5) holds by construction. Five offender classes: `E1 UNKNOWN_TOKEN` (a `var(--x)` in no resolvable catalog, reported with the nearest real token by edit distance), `E2 VAR_FALLBACK` (`var(--x, ...)` banned outright, because the fallback renders a plausible value and makes a fabricated token invisible), `E3 UNKNOWN_MOTION` (duration and easing literals absent from the Duration Scale and Easing Curves), `E4 IMG_NO_ALT`, and `E5 INPUT_NO_LABEL`. The token allowlist is the union of the design system's token tables, every `--x:` declared under `:root` / `.dark` / `@theme` in project CSS, and `tailwind.config.*` bindings; if none of the three resolves it exits **2** with a loud message rather than reporting a clean pass against an empty allowlist. Exit 0 clean, 1 with offenders. No network, no installs, deterministic, POSIX-portable awk. Carries a `--self-test` flag running 18 built-in fixtures.
- **Placement is load-bearing and needs zero manifest edits.** `package.json`'s `files` allowlists `scripts/install`, not `scripts/`, so a root-level script would not ship over npm. It allowlists `skills`, and `"skills": "./skills"` is already present in both plugin manifests, so a script under `skills/sell-slice/scripts/` ships on both clients and both install paths. Nothing scans `scripts/` and no manifest key points at it: a bundled script runs because a skill body names the command, which is exactly how `slice-verifier` §4 invokes this one.

### Changed

- **`slice-verifier` §4 gains the sub-check and an exit-2 branch.** Exit 1 is `design_system: fail` with each `file:line` offender copied into `fix_targets`. Exit 2 (no token catalog resolvable) is neither pass nor fail: it returns `needs_human` with `prd_ambiguity`, because a design-system gate with no catalog behind it can only ever emit a false green.
...
- **`verify-frontend-statics.sh` E1 checks `var(--x)` references only, never bare utility class names.** Separating a Tailwind built-in (`bg-slate-500`, `text-sm`, `border-2`) from a fabricated token would mean embedding Tailwind's entire palette and scale set, a permanent drift surface, and raw utilities are already covered by `slice-verifier`'s existing raw-value grep. The documented failure this closes (`bg-[var(--brand-primary-500)]` against a real `--primary`) is the arbitrary-value form, which is caught.
- **E4 and E5 are the only two a11y patterns in the script, deliberately.** They are source patterns, not a rendered pass, and they do not make `slice-verifier` an accessibility gate. The rendered pass empirically catches focus suppression and Tab-unreachability but not a missing accessible name on an image, which is why these two earn their place at the cost of two regexes in a script already running.
