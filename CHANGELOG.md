...
- **A rendered accessibility pass in `slice-tester` (§2a), blocking.** Seven checks against surfaces the agent is already screenshotting: `contrast_wcag_aa` (computed foreground against effective background, 4.5:1 body and 3:1 large text and non-text boundaries), `keyboard_nav` (Tab from `document.body` until focus cycles, asserting every manifest affordance appears and the order tracks bounding-box reading order), `focus_visible` (computed outline/box-shadow/border diffed against unfocused, where a null diff is a fail), `focus_trap`, `error_not_color_only`, `prefers_reduced_motion`, and `dark_mode_parity`. Appendix B gains the `a11y` block and `tooling_tier_used`. **Any a11y fail forces `overall: fail`.** Kept blocking deliberately: an interface a keyboard cannot reach is not working software, and downgrading accessibility to advisory is how the check dies.
- **`tooling_tier_used` on `slice-tester`'s contract** (`claude_in_chrome` | `chrome_devtools_mcp` | `playwright` | `vizzly`). The agent already knows which tier it drove, because it just drove it; it was simply never asked. One line, and silent degradation becomes a fact the pie boundary can read. It is a recorded observation, not a health check, so no tool is ever probed to populate it.
- **A signature bail in both fix loops.** `fix-attempter` and `ci-fix-attempter` compute `signature = sha1(normalize(first_error_line) + '|' + file + '|' + check_id)`, where `normalize` strips line and column numbers, absolute path prefixes and timestamps. The orchestrator threads the prior attempt's value in as `previous_signature`; on a match the agent **patches nothing** and returns `bail_reason: repeated_signature` with `needs_human: true`. String equality, not judgment. `box-it-up` already asserted in prose that three identical-cause failures signal a structural problem and then implemented a plain counter, so three identical failures burned the same three expensive loops as three different ones. This is a **net latency saving**.
- **`regressed_checks` and a revert rule.** The orchestrator diffs each re-verification against the previous verdict and passes any check that was green before the patch and is red after. The fixer then reverts its own last patch rather than stacking a second fix on top of it, returning `bail_reason: regression_reverted`. Nothing previously reverted a fix that turned a green check red.

### Changed

- **Carry-forward in `slice-verifier` is now mechanical.** The old rule ("only re-run a check the fixer's last patch could plausibly have changed") was an unaided judgment. It becomes set intersection: re-run check `C` if and only if `C`'s declared input file set intersects `fix-attempter.files_changed[]`, with the sets tabulated per check (lint/typecheck/build = all changed files; unit_integration = changed files plus test siblings; design_system = changed `.tsx`/`.jsx`/`.css`; ci_integrity = changed `.github/workflows/**`; manifest_backstop = any changed file). Strictly fewer re-runs than a model erring conservative, verify-once (C5) preserved by construction, and it is what makes a regression detectable at all.
- **The browser-tooling ladder now lives in `slice-tester` in full**, instead of being anchored to `frontend/visual-reviewer.md`. That file is deprecated and absent from `agents[]`, so the highest-value frontend gate had a live dependency on a file scheduled for deletion.
- **Hover assertions record `pass_discrete`, not `pass`.** A dispatched `pointerenter` is a point, not a cursor path, and the existing frontend row asserted "hover tooltips produce a state change" as though it were the latter.
- **`fix-attempter`'s description no longer names deprecated agents.** It still cited `basic-checks-runner` and `aggregating-test-reviewer`, both folded into `slice-verifier` and `slice-tester` in v5.
- **`state-illustrator`'s `prefers-reduced-motion` constraint now names the live check that catches it.** It previously deferred to the deprecated `visual-reviewer`, which never loads.
...
- **Expect the frontend fail rate to rise on the first pie or two after adopting the a11y pass.** Focus-ring suppression and unnamed icon buttons are endemic in generated UI, and every failure routes into a fix loop capped at 3. That is the check working, not the check misfiring. If it thrashes, record a one-pie grace period rather than softening the verdict; downgrading accessibility to advisory is how the check dies.
- **A green a11y verdict covers the machine-checkable band only, roughly 30 to 50 percent of WCAG.** Screen-reader behavior on VoiceOver, NVDA, JAWS and TalkBack is not automatable and is never implied by the result. This is stated once in `slice-tester`'s Hard Constraints rather than emitted as a per-slice advisory block: text with zero variance carries zero information.
